...
export class Bar implements HTTPResponseInterceptor {
  onResponse(res: AxiosResponse): AxiosResponse | Promise<AxiosResponse>  {/** intercept logic */}
}
...
export class Foo implements HTTPRequestInterceptor {
  ...
  ): AxiosRequestConfig | Promise<AxiosRequestConfig> {/** intercept logic */}
}
```
...
```ts
import {HTTPRequestInterceptor, AxiosRequestConfig, HTTPFactory} from '@wizardoc/fence';
